<div align="center">
  <img src="https://github.com/user-attachments/assets/2fd40a17-7fad-446a-bacc-4dc73bd4cc05" width="220" alt="Isabel — Web Developer" />
  <h1>¡Hola! Soy Isabel 💻✨</h1>
  <p>Desarrolladora Web especializada en <b>DAW (Desarrollo de Aplicaciones Web)</b></p>
  <p>Analítica, observadora y con un enfoque estructurado para resolver problemas lógicos.</p>
</div>

---

## 🛠️ Mi Caja de Herramientas & Proyectos

### ⚙️ Arquitectura Backend & Bases de Datos

<table>
  <tr>
    <td width="35%" align="center" valign="top">
      <img src="https://github.com/user-attachments/assets/e44157a5-d3c6-4945-bca0-33b1a98785ea" width="180" alt="Isabel trabajando en Backend" />
    </td>
    <td width="65%" valign="top">
      <p>En el desarrollo del lado del servidor, me enfoco en construir arquitecturas limpias,responsivas, escalables y seguras. Disfruto estructurando bases de datos robustas, creando lógica eficiente y optimizando el flujo de los datos.</p>
      <p>
        <img src="https://img.shields.io/badge/php-%23777BB4.svg?style=flat-square&logo=php&logoColor=white" alt="PHP" /> 
        <img src="https://img.shields.io/badge/laravel-%23FF2D20.svg?style=flat-square&logo=laravel&logoColor=white" alt="Laravel" /> 
        <img src="https://img.shields.io/badge/python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python" /> 
        <img src="https://img.shields.io/badge/mysql-%2300f.svg?style=flat-square&logo=mysql&logoColor=white" alt="MySQL" />
      </p>
       <h4>🏥 Proyecto Destacado: Medical-Iberia</h4>
      <p><b>Estado:</b> <i>Colaboración activa en equipo aplicando metodologías ágiles y Git.</i></p>
      <ul>
        <li>Sistemas de localización y traducción multiidioma nativo en Laravel.</li>
        <li>Desarrollo de widgets para control de eventos RFID con Filament.</li>
        <li><b>Integración de scripts en Python</b> para la automatización, lectura y procesamiento de flujos de datos lógicos.</li>
        <li><b>Identificación y resolución de bugs críticos (Refactorización)</b>, optimizando la carga y mejorando drásticamente la fluidez de la aplicación.</li>
        <li>Control de versiones riguroso con Git trabajando en equipo.</li>
      </ul>
      <br/>
      <a href="https://github.com/isabelbv/medical-iberia">
        <img src="https://github-readme-stats.vercel.app/api/pin/?username=isabelbv&repo=medical-iberia&theme=visual_studio_code&show_owner=false" alt="Medical Iberia Repo" />
      </a>
    </td>
  </tr>
</table>

---

### 💻 Interfaces Frontend & Diseño Dinámico

<table>
  <tr>
    <td width="65%" valign="top">
      <p> Busco la precisión estética y el equilibrio visual sin comprometer la velocidad ni la experiencia de usuario. Me gusta dar vida a interfaces dinámicas, intuitivas y altamente modulares.</p>
      <p>
        <img src="https://img.shields.io/badge/html5-%23E34F26.svg?style=flat-square&logo=html5&logoColor=white" alt="HTML5" /> 
        <img src="https://img.shields.io/badge/css3-%231572B6.svg?style=flat-square&logo=css3&logoColor=white" alt="CSS3" /> 
        <img src="https://img.shields.io/badge/javascript-%23F7DF1E.svg?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript" /> 
        <img src="https://img.shields.io/badge/react-%2320232a.svg?style=flat-square&logo=react&logoColor=%2361DAFB" alt="React" /> 
        <img src="https://img.shields.io/badge/angular-%23DD0031.svg?style=flat-square&logo=angular&logoColor=white" alt="Angular" />
      </p>
     <h4>🛒 Proyecto Destacado: Dulcisa (TFG)</h4>
      <p><b>Estado:</b> <i>En fase final de optimización y despliegue para Proyecto de Fin de Grado.</i></p>
      <ul>
        <li>Plataforma e-commerce completa para una pastelería tradicional con diseño de interfaz 100% <b>Responsive</b>.</li>
        <li>Frontend desarrollado en React implementando <b>Hooks personalizados</b> para una gestión de estado limpia y eficiente.</li>
        <li>Consumo e integración de <b>APIs REST</b> para la comunicación asíncrona con el backend (control de carrito, stock, alérgenos y pedidos).</li>
      </ul>
      <br/>
      <a href="https://github.com/isabelbv/dulcisa-web">
        <img src="https://github-readme-stats.vercel.app/api/pin/?username=isabelbv&repo=dulcisa-web&theme=visual_studio_code&show_owner=false" alt="Dulcisa Repo" />
      </a>
    </td>
    <td width="35%" align="center" valign="top">
      <img src="https://github.com/user-attachments/assets/a1ac0527-2a3d-4736-ad7a-b4b2e0341c00" width="180" alt="Isabel programando Frontend" />
    </td>
  </tr>
</table>

---

## 🧰 Tecnologías & Stack Tecnológico

<div align="center">
  <br/>
  
  ### 🌐 Desarrollo Frontend
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" alt="React" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="JavaScript" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/angularjs/angularjs-original.svg" alt="Angular" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" alt="HTML5" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" alt="CSS3" width="55" height="55"/>
  
  <br/><br/>

  ### ⚙️ Desarrollo Backend, APIs & Lógica
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/laravel/laravel-original.svg" alt="Laravel" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/php/php-original.svg" alt="PHP" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="Python" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" alt="Java" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/postman/node-postman/master/assets/postman-logo.svg" alt="APIs / Postman" width="55" height="55"/>

  <br/><br/>

  ### 🗄️ Bases de Datos, Sistemas & Metodologías
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" alt="MySQL" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" alt="Linux" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/ubuntu/ubuntu-plain.svg" alt="Ubuntu" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/apache/apache-original.svg" alt="Apache" width="55" height="55"/>&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="Git" width="55" height="55"/>

</div>

---

### 🔧 Buenas Prácticas & Entornos de Servidor
* **Metodologías y Arquitectura:** `Programación Orientada a Objetos (POO)` &bull; `Patrón MVC` &bull; `Clean Code` &bull; `Desarrollo de APIs REST`
* **Gestión de Servidores y Sistemas:** `Virtualización (VirtualBox)` &bull; `Configuración de Servidores Web (Apache2)` &bull; `Gestión de Directorios (LDAP)` &bull; `GitFlow`

---

## 📈 Estadísticas de GitHub

<div align="center">
  <a href="https://github.com/anuraghazra/github-readme-stats">
    <img src="https://github-readme-stats.vercel.app/api?username=isabelbv&show_icons=true&theme=visual_studio_code" alt="GitHub Readme Stats" />
  </a>
</div>

---

## 💡 Cita / Chiste Dev del Día

<div align="center">
  <img src="https://readme-jokes.vercel.app/api?theme=vscode&bgColor=1e1e1e" alt="Chiste para programadores dinámico" />
</div>


## 📫 Conecta conmigo

* **LinkedIn:** [Isabel de Blas Viñas](https://www.linkedin.com/in/isabel-de-blas-vi%C3%B1as-2093b2188/)
* **Email:** isabelblasv@gmail.com
